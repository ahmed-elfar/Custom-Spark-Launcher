...
public class SparkLauncher extends AbstractLauncher<SparkLauncher> {
  ...
  public SparkLauncher setSparkHome(String sparkHome) {
    ...
    builder.childEnv.put(CommandBuilderUtils.ENV_SPARK_HOME, sparkHome);
    return this;
  }
  ...
  public SparkLauncher splitStreams() {
    onlyStreams = true;
    return this;
  }
   ...
   * <p>
   * The {@link #startApplication(org.apache.spark.launcher.SparkAppHandle.Listener...)} method is preferred when launching
   ...
   *
   * @since 1.6.0
   * @see AbstractLauncher#startApplication(org.apache.spark.launcher.SparkAppHandle.Listener...)
   * @param listeners Listeners to add to the handle before the app is launched.
   * @return A handle for the launched application.
   */
  @Override
  public org.apache.spark.launcher.SparkAppHandle startApplication(org.apache.spark.launcher.SparkAppHandle.Listener... listeners) throws IOException {
    ...
    boolean outputToLog = outputStream == null || !onlyStreams;
    boolean errorToLog = (!redirectErrorStream && errorStream == null) || !onlyStreams;

    // Only setup stderr + stdout to logger redirection if user has not otherwise configured output
    // redirection.
    if (loggerName == null && (outputToLog || errorToLog) && !onlyStreams) {
      String appName;
    ...
    if (outputToLog && errorToLog && !onlyStreams) {
      pb.redirectErrorStream(true);
    }
    ...
    try {
      ...
      if (loggerName != null && !onlyStreams) {
        logStream = outputToLog ? child.getInputStream() : child.getErrorStream();
      }
      handle.setChildProc(child, loggerName, logStream, onlyStreams);
  ...
  private ProcessBuilder createBuilder() throws IOException {
    ...
    CommandBuilderUtils.checkState(!redirectErrorStream || errorStream == null,
      "Cannot specify both redirectError() and redirectError(...) ");
    CommandBuilderUtils.checkState(getLoggerName() == null ||
  ...
  String findSparkSubmit() {
    ...
    return CommandBuilderUtils.join(File.separator, builder.getSparkHome(), "bin", script);
  }
