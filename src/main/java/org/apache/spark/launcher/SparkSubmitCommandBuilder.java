...
class SparkSubmitCommandBuilder extends AbstractCommandBuilder {
  ...
  List<String> buildSparkSubmitArgs() {
    ...
    if (!allowsMixedArguments && !isSpecialCommand) {
      CommandBuilderUtils.checkArgument(appResource != null, "Missing application resource.");
    }
    ...
    if (!jars.isEmpty()) {
      ...
      args.add(CommandBuilderUtils.join(",", jars));
    }

    if (!files.isEmpty()) {
      args.add(parser.FILES);
      args.add(CommandBuilderUtils.join(",", files));
    }

    if (!pyFiles.isEmpty()) {
      args.add(parser.PY_FILES);
      args.add(CommandBuilderUtils.join(",", pyFiles));
    }

    if (isExample && !isSpecialCommand) {
      CommandBuilderUtils.checkArgument(mainClass != null, "Missing example class name.");
    }
  ...
  private List<String> buildSparkSubmitCommand(Map<String, String> env)
    ...
    if (isClientMode) {
      ...
      String memory = CommandBuilderUtils.firstNonEmpty(tsMemory, config.get(SparkLauncher.DRIVER_MEMORY),
        System.getenv("SPARK_DRIVER_MEMORY"), System.getenv("SPARK_MEM"), CommandBuilderUtils.DEFAULT_MEM);
      cmd.add("-Xmx" + memory);
      addOptionString(cmd, driverDefaultJavaOptions);
      addOptionString(cmd, driverExtraJavaOptions);
      CommandBuilderUtils.mergeEnvPathList(env, CommandBuilderUtils.getLibPathEnvName(),
  ...
  private List<String> buildPySparkShellCommand(Map<String, String> env) throws IOException {
    ...
    CommandBuilderUtils.checkArgument(appArgs.isEmpty(), "pyspark does not support any application options.");
    ...
    pyargs.add(CommandBuilderUtils.firstNonEmpty(conf.get(SparkLauncher.PYSPARK_DRIVER_PYTHON),
  ...
  private List<String> buildSparkRCommand(Map<String, String> env) throws IOException {
    ...
    env.put("R_PROFILE_USER",
            CommandBuilderUtils.join(File.separator, sparkHome, "R", "lib", "SparkR", "profile", "shell.R"));

    List<String> args = new ArrayList<>();
    args.add(CommandBuilderUtils.firstNonEmpty(conf.get(SparkLauncher.SPARKR_R_SHELL),
  ...
  boolean isClientMode(Map<String, String> userProps) {
    String userMaster = CommandBuilderUtils.firstNonEmpty(master, userProps.get(SparkLauncher.SPARK_MASTER));
    String userDeployMode = CommandBuilderUtils.firstNonEmpty(deployMode, userProps.get(SparkLauncher.DEPLOY_MODE));
  ...
  private List<String> findExamplesJars() {
    ...
    CommandBuilderUtils.checkState(isTesting || foundDir, "Examples jars directory '%s' does not exist.",
  ...
  private class OptionParser extends SparkSubmitOptionParser {
    ...
    protected boolean handleUnknown(String opt) {
      ...
      if (allowsMixedArguments) {
        ...
        CommandBuilderUtils.checkState(appResource == null, "Found unrecognized argument but resource is already set.");
        appResource = opt;
        return false;
      }
      return true;
